...
**The 3D panel, in progress.** `/panel` is the first stage of a rebuild of the deck as a real WebGL scene (react-three-fiber and drei, lazy-loaded, with the CSS deck as the fallback where WebGL is missing or the first frames are slow): one push-light on a painted plate with a screw in each corner, lit and unlit, that depresses under a tap and buzzes the phone; the panel leans a few degrees with the phone (or the mouse), frames are drawn only when something changes, the pixel ratio is capped at 1.5 and lowered further on a weak device, and labels are set in B612. The studio environment map is served from `web/public/env`. `?css=1` shows the fallback.
