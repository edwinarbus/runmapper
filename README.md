...
**The deck as a real panel.** Where the browser can draw WebGL, the setup column is a three-dimensional panel (react-three-fiber and drei, in a chunk fetched after the page is up; the CSS deck shows until it arrives, and instead wherever WebGL is missing, the first frames come too slowly, or `?css=1` asks for it): five painted plates with a screw in each corner, and every control built from code. A rotary knob picks words, a drawing or an image; the word is typed into a fourteen-segment LED readout (real segments, not a font, under dark glass) through an invisible field laid over it, so the keyboard works as it always did; the address field, the draw pad and the image picker stay real HTML laid over their plates. Three push-lights choose the distance (the ones a word does not fit stay dark), bat-handle toggles set miles or kilometres and loop or one way, a second knob sets the style, and a toggle under a red guard starts the search: lift the guard, throw the switch, and a gauge follows the engine's progress while a readout scrolls what it is doing; throw it back to stop. Throws buzz the phone; the panel leans a few degrees with the phone (or the mouse); frames are drawn only when something changes; the pixel ratio is capped at 1.5 and lowered, with the reflections, on a weak device; labels are set in B612 and the studio environment map is served from `web/public/env`. `/panel` keeps the first stage, one push-light on a plate, as a check page.
