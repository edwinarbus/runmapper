...
- **Placement**: streets within about a mile and a half of the pin come from Overpass. The drawing is tried at the pin first: the local grid angle and block spacing are measured, a few hundred placements (position × size × rotation) are scored by how close the ideal strokes lie to real streets, and the best few are snapped with a corridor-restricted Dijkstra that hugs each stroke, joined by connectors that retrace existing ink, and closed into a loop when asked. If that is not a good fit, spots up to about 0.9 mi away whose streets form a clearly more regular grid are tried, nearest first, and the search stops at the first band of distance that gives a good fit. So the run starts as close to the pin as a good drawing allows: at the pin when it is within a quarter mile by street (the walk-on is built into the route), otherwise at the drawing, with the distance from the pin reported.
